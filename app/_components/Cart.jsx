...
function Cart({ openCart, setOpenCart }) {
  ...
  // },[cart])
  return cart && (
    <div className='h-[300px] w-[250px] bg-gray-100 z-10
     rounded-md absolute mx-10 right-10 top-12 p-5 overflow-auto'>
        ...
        <ul className="space-y-4">
          {cart.map((item, index) => (
            <li className="flex items-center gap-4">
              <img
                src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL}${item?.product?.attributes?.banner?.data?.attributes?.url}`}
                alt=""
